...
scheduler(void)
{
  ...
  p->wait_ticks = 0;
    ...
    if(highp){
      ...
      switchuvm(highp);
      highp->wait_ticks = 0;
      highp->run_count++;          // count scheduler dispatches
      highp->state = RUNNING;
...
// Fetch process info for all processes
int
getpinfo(struct pinfo *table)
{
  int count = 0;
  struct proc *p;
  struct proc *cp;
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    ...
    table[count].sz        = p->sz;
    table[count].run_count = p->run_count;
    table[count].runtime   = ticks - p->start_ticks;  // ticks alive
    safestrcpy(table[count].name, p->name, sizeof(p->name));

    // Traverse process table to find children of this process
    int nc = 0;
    for(cp = ptable.proc; cp < &ptable.proc[NPROC]; cp++){
      if(cp->parent == p && cp->state != UNUSED && nc < 64)
        table[count].children[nc++] = cp->pid;
    }
    table[count].nchildren = nc;
